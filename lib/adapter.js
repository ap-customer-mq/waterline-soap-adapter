  ...
  var adapter = {
    ...
    request: function(identity, collection, actionName, args, context, cb) {
      ...
      soap.createClient(wsdlUrl, wsdlOptions, function(err, client) {
        ...
        if (wsSecurity) {
          var _username, _password, _passwordType;
          _username = interpolate(wsSecurity.username, context);
          _password = interpolate(wsSecurity.password, context);
          _passwordType = interpolate(wsSecurity.passwordType, context);

          client.setSecurity(new WSSecurity(_username, _password, _passwordType, wsSecurity.useTimestamps || false));
        }
